// 8-trailglow — estela (trail) + halo pulsante (glow) en un solo shader.
// Es el combo que ya tenías montado.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // --- Glow pulsante sobre el resultado del trail ---
    vec2 cc = vec2(currentCursor.x + currentCursor.z * 0.5, currentCursor.y - currentCursor.w * 0.5);
    float dg = getSdfRectangle(vu, cc, currentCursor.zw * 0.5);
    float pulse = mix(0.45, 1.0, 0.5 + 0.5 * sin(iTime * 4.0));
    float glow = exp(-max(dg, 0.0) / 0.05) * 0.55 * pulse * step(0.0, dg);
    fragColor += vec4(GLOW_COLOR * glow, 0.0);
}
